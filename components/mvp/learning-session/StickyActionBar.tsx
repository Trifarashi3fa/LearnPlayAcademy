...
export function StickyActionBar({
  ...
  return (
    <nav
      className="min-h-[4.35rem] border-t border-[#DDE8F5] bg-white/95 px-2 pt-2 shadow-[0_-8px_24px_rgba(8,43,128,0.1)] backdrop-blur sm:px-5 sm:pt-2.5"
      style={{ paddingBottom: "max(0.5rem, env(safe-area-inset-bottom))" }}
      aria-label="Question actions"
    >
      <div className="mx-auto grid max-w-[100rem] min-w-0 grid-cols-[auto_1fr] items-center gap-2 sm:flex sm:gap-3">
        ...
        <MvpButton
          onClick={onNext}
          disabled={!answered}
          className="col-span-2 min-w-0 px-4 transition-transform duration-200 motion-safe:hover:-translate-y-0.5 sm:col-span-1 sm:flex-none sm:min-w-48 xl:min-w-56"
        >
