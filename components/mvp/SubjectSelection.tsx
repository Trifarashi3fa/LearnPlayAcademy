...
import type { AnalyticsSubject } from "@/lib/learning-analytics/catalog";

function toAnalyticsSubject(subjectId: string): AnalyticsSubject {
  if (subjectId === "mathematics" || subjectId === "english" || subjectId === "science") return subjectId;
  return "unknown";
}

export function SubjectSelection() {
  const { worldProgress, completedCount } = useMvpProgress();
  const activeSubjectCount = publicSubjectAvailability.filter((subject) => isFeatureActive(subject.featureId)).length;
  const comingSoonSubjectCount = publicSubjectAvailability.length - activeSubjectCount;

  useEffect(() => {
    trackLearningEvent("subject_selection_viewed", { activeSubjectCount, comingSoonSubjectCount });
  }, [activeSubjectCount, comingSoonSubjectCount]);
      ...
      {publicSubjectAvailability.map((subject) => {
        ...
        return active ? (
          <Link
            key={subject.id}
            href={subject.href}
            onClick={() => trackLearningEvent("subject_card_clicked", {
              subject: toAnalyticsSubject(subject.id),
              active: true,
              destination: subject.href,
            })}
