...
export function QuestionCard({ question, visual, selectedAnswer, onSelectAnswer, compact = false }: QuestionCardProps) {
  ...
  const compactVisual = compact || isComparisonQuestion;

  return (
    <section
      className={`flex h-full min-h-0 flex-col border border-[#DDE8F5] bg-white shadow-playful transition-all duration-200 motion-reduce:transition-none ${compact ? "rounded-[1.5rem] p-2.5 sm:p-3" : "rounded-[2rem] p-5 sm:p-7"}`}
    ...
    >
      ...
      <h2
        ...
        className={`mt-1.5 shrink-0 font-black leading-tight text-[#082B80] ${isComparisonQuestion ? "text-[1.22rem] sm:text-[1.4rem] xl:text-[1.5rem]" : compact ? "text-[1.28rem] sm:text-[1.65rem] xl:text-[1.9rem]" : "text-3xl sm:text-4xl"}`}
      >
        {question.question}
      </h2>

      <div className={`${isComparisonQuestion ? "mt-1.5 p-1.5 sm:p-2" : "mt-2 p-1.5 sm:p-2"} min-h-0 shrink overflow-visible rounded-[1.25rem] border border-[#DDE8F5] bg-[#F8FBFF] transition-all duration-200 motion-reduce:transition-none`}>
        <MathVisualRenderer visual={visual} revealAnswer={answered} compact={compactVisual} fitNarrow />
      </div>

      <div className={`${isComparisonQuestion ? "mt-1.5 sm:mt-2" : "mt-2"} shrink-0`}>
        <QuestionRenderer
