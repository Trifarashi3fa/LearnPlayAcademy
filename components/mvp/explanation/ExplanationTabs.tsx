  ...
  return (
    <section className={`flex min-h-0 flex-col overflow-hidden bg-white ${compact ? "h-full rounded-[1.25rem]" : "rounded-[2rem] border border-[#DDE8F5] shadow-sm"}`}>
      <div className={`shrink-0 border-b border-[#DDE8F5] ${compact ? "p-3" : "p-5 sm:p-6"}`}>
        ...
        <div className={`${compact ? "grid grid-cols-2 gap-2" : "mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap"}`} role="tablist" aria-label="Question explanation">
          {tabs.map((tab) => (
            <button key={tab.id} id={`explanation-tab-${tab.id}`} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`explanation-panel-${tab.id}`} onClick={() => setActiveTab(tab.id)} className={`min-h-12 rounded-full px-3 py-2 text-sm font-black transition focus:outline-none focus:ring-4 focus:ring-[#0B63F6]/25 ${activeTab === tab.id ? "bg-[#0B63F6] text-white" : "bg-[#EAF6FF] text-[#082B80] hover:bg-[#DCEEFF]"}`}>{tab.label}</button>
          ))}
        </div>
      </div>
      <div id={`explanation-panel-${activeTab}`} role="tabpanel" aria-labelledby={`explanation-tab-${activeTab}`} className={`min-h-0 flex-1 overflow-y-auto overscroll-contain ${compact ? "p-3" : "min-h-80 p-5 sm:p-7"}`}>
